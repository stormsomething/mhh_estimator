...
if __name__ == '__main__':
    ...
    rnn_mmc_comparison(all_predictions, all_fold_1_arrays['universal_true_mhh'], dihiggs_01, all_fold_1_arrays, 'all', args.library, predictions_mmc = all_mmc, sigma_label = '_low_sigma', sigma_slice = indices_1_all)
    rnn_mmc_comparison(all_predictions, all_fold_1_arrays['universal_true_mhh'], dihiggs_01, all_fold_1_arrays, 'all', args.library, predictions_mmc = all_mmc, sigma_label = '_high_sigma', sigma_slice = indices_2_all)
    ...
    for i in range(len(dihiggs_01.fold_1_array['universal_true_mhh'])):
        ...
        if ((reweight_bin > -1) and (reweight_bin < num_bins)):
            new_weights.append(dihiggs_01.fold_1_array['EventInfo___NominalAuxDyn']['evtweight'][i] * dihiggs_01.fold_1_array['fold_weight'][i] * reweights_by_bin[reweight_bin])
        else:
            new_weights.append(0)
    new_weights = np.array(new_weights)
    reweight_plot(dihiggs_01.fold_1_array['universal_true_mhh'], dihiggs_10.fold_1_array['universal_true_mhh'], dihiggs_01.fold_1_array, dihiggs_10.fold_1_array, new_weights, 'truth')
    ...
    z = np.sqrt(z1 * z1 + z2 * z2)
    original_significances.append(z)
    z1 = k_lambda_comparison_plot(dihiggs_10.fold_1_array[indices_1_10]['universal_true_mhh'], dihiggs_01.fold_1_array['universal_true_mhh'][indices_1_1], dihiggs_10.fold_1_array[indices_1_10], dihiggs_01.fold_1_array[indices_1_1], 'truth_good_reversed', k10mode = True)
    z2 = k_lambda_comparison_plot(dihiggs_10.fold_1_array[indices_2_10]['universal_true_mhh'], dihiggs_01.fold_1_array['universal_true_mhh'][indices_2_1], dihiggs_10.fold_1_array[indices_2_10], dihiggs_01.fold_1_array[indices_2_1], 'truth_bad_reversed', k10mode = True)
    z = np.sqrt(z1 * z1 + z2 * z2)
    original_significances.append(z)
